...
)
{
	...
	for (size_t I = 0; I < Targets.size (); I = I + 1)
	{
		...
		{
			Distance = (float)cv::norm (Trackers [J].P - Targets [I].pt);
...
)
{
	for (size_t I = 0; I < Trackers.size (); I = I + 1)
	{
		...
		int Duration = Trackers [I].Duration;
		...
		float Angle = (180.0f / (float)M_PI) * Trackers [I].Angle;
		...
		if (Counted)
		{
			...
			printf ("\n");
			fflush (stdout);
			//Trackers [I].P = {LEP3_W / 2.0f, LEP3_H/2.0f};
