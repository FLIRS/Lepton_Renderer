...
(
	...
	size_t pix_count, 
	struct Pixel_ABGR8888 const * pallete,
	size_t pallete_count
)
{
	for (size_t i = 0; i < pix_count; i = i + 1)
	{
		ASSERT_F (source [i] < pallete_count, "%i < %i", source [i], pallete_count);
		destination [i].r = pallete [source [i]].r;
		destination [i].g = pallete [source [i]].g;
		destination [i].b = pallete [source [i]].b;
		destination [i].a = 255;
	}
}


void map_indexed_float_ABGR8888 
(
   float const * source, 
   struct Pixel_ABGR8888 * destination, 
   size_t pix_count, 
...
)
{
   for (size_t i = 0; i < pix_count; i = i + 1)
   {
      size_t j = (int) map_lin_float (source [i], min, max, 0.0f, (float) pallete_count - 1.0f);
      assert (j < pallete_count);
      destination [i].r = pallete [j].r;
      destination [i].g = pallete [j].g;
